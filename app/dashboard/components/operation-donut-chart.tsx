...
// Operation mapping function with theme-aware colors
...
interface OperationDonutChartProps {
  data: Transaction[]
}

export function OperationDonutChart({ data }: OperationDonutChartProps) {
  // Process data to get operation spending
  const chartData = React.useMemo(() => {
    const operationTotals = data.reduce(
      (acc, transaction) => {
        if (transaction.type === "OUTFLOW") {
          const operation = transaction.operation_type
          acc[operation] = (acc[operation] || 0) + transaction.amount
        }
        return acc
      },
  ...
  console.log("Operation donut chart data:", chartData)

  if (chartData.length === 0) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>Gastos por Operação</CardTitle>
          <CardDescription>
            Distribuição dos gastos nos últimos 3 meses
          </CardDescription>
        </CardHeader>
        <CardContent>
              ...
              <PieChart>
                ...
                >
                  {chartData.map((entry, index) => (
                    <Cell
                      key={`cell-${index}`}
                      fill={`url(#operation-gradient-${index})`}
                    />
                  ))}
                  <Label
                    content={({ viewBox }) => {
                  ...
                  content={
                    <ChartTooltipContent
                      ...
                      labelFormatter={(value, payload) => {
                        console.log("Operation tooltip labelFormatter:", {
                          value,
                          payload,
                        })
                      ...
                      formatter={(value, name, props) => {
                        console.log("Operation tooltip formatter:", {
                          value,
                          name,
                          props,
                        })
